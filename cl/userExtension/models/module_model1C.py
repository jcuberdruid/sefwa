...
def model(numLabels):
    ...
    # Conv3D layer
    model.add(Conv3D(filters=32, kernel_size=(5, 3, 3), strides=(1, 1, 1), padding='valid', data_format='channels_last', activation='relu', name='conv3d2'))
    # Batch normalization
    ...
    model.add(Dense(units=512, activation='relu', kernel_regularizer=l1(0.005), name='dense'))
    model.add(Dropout(rate=0.5, name="dropout_2"))
    model.add(Dense(units=256, activation='relu', kernel_regularizer=l1(0.004), name='dense_1'))
