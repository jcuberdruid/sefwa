...
from channelSVM import ChannelSVM
import numpy as np
import pandas as pd
...
def main():
    ...
    eeg_handler_class_1 = EEGDataHandler(class_1_path, slice_index, 1) 
    eeg_handler_class_2 = EEGDataHandler(class_2_path, slice_index_2, 1)

    # Split data into training, validation, and testing sets
    cl1_train, cl1_val, cl1_test = eeg_handler_class_1.split_data(train_size=0.8, test_size=0.1, random_state=42)
    cl2_train, cl2_val, cl2_test = eeg_handler_class_2.split_data(train_size=0.8, test_size=0.1, random_state=42)

    # Number of electrodes/channels
    num_channels = 64

    # Using multiprocessing to train SVM models in parallel
    num_processes = 2  # Using half of the available CPU cores
    svm_models = []
    with Pool(num_processes) as pool:
        # Pass additional data needed by train_and_evaluate_channel
        args_for_pool = [(channel, cl1_train, cl2_train, cl1_val, cl2_val, cl1_test, cl2_test) for channel in range(num_channels)]
        results = pool.starmap(train_and_evaluate_channel, args_for_pool)
        for i, svm in enumerate(results, 1):
            svm_models.append(svm)
            #print(f"Channel {i}/{num_channels} processed")
    ...
    csv_filename = f"results40in_40wide/results_{os.path.basename(class_1_path)}_{os.path.basename(class_2_path)}_slice_{slice_index}"
